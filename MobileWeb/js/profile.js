...
$(function () {
    ...
    const badgeMap = {
        ...
        'Hoàn thành':    { cls: 'badge-success', icon: 'fa-circle-check' },
        'Đang giao':     { cls: 'badge-info',    icon: 'fa-truck' },
        'Đã xác nhận':   { cls: 'badge-info',    icon: 'fa-circle-check' },
    ...
    function openOrderDetail(maHD) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // Action buttons - chỉ hiện nút "Hủy đơn" khi trạng thái = "Chưa xác nhận"
                if (hd.TrangThai === 'Chưa xác nhận') {
                    $('#orderDetailActions').html(`
                        <button class="btn btn-danger-solid" id="btnCancelOrder" data-mahd="${maHD}">
                            <i class="fa-solid fa-circle-xmark"></i> Hủy đơn hàng
                        </button>
                    `).show();
                } else {
                    $('#orderDetailActions').html('').hide();
                }
            },
            error: function () {
    ...
    // Click nút "Hủy đơn hàng" trong modal chi tiết
    $(document).on('click', '#btnCancelOrder', function () {
        const maHD = $(this).data('mahd');

        if (!confirm('Bạn có chắc muốn hủy đơn hàng #' + maHD + ' không?\n\nHành động này không thể hoàn tác.')) {
            return;
        }

        const $btn = $(this);
        $btn.html('<i class="fa-solid fa-spinner fa-spin"></i> Đang hủy...').prop('disabled', true);
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    showToast('✅ ' + res.message, 'success');
                    closeOrderDetail();
                    // Reload trang để cập nhật bảng đơn hàng
                    setTimeout(() => location.reload(), 1000);
                } else {
                    showToast('❌ ' + res.message, 'error');
                    $btn.html('<i class="fa-solid fa-circle-xmark"></i> Hủy đơn hàng').prop('disabled', false);
                }
            },
            error: function () {
