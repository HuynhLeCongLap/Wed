...
if (isset($_POST['action']) && $_POST['action'] === 'cancel_order') {
    ...
    // Kiểm tra đơn có thuộc về user và đang ở trạng thái cho phép hủy
    $chk = $conn->prepare("SELECT TrangThai FROM hoadon WHERE MaHoaDon = ? AND TenDangNhap = ?");
    $chk->bind_param('is', $ma_hd, $username);
    $chk->execute();
    $hd = $chk->get_result()->fetch_assoc();
    ...
    if ($hd['TrangThai'] !== 'Chưa xác nhận') {
        echo json_encode([
            'success' => false,
            'message' => 'Chỉ có thể hủy đơn ở trạng thái "Chưa xác nhận". Đơn này đang "' . $hd['TrangThai'] . '".'
        ]);
        exit;
    }

    // Update trạng thái (kèm điều kiện chống race condition)
    $stmt = $conn->prepare(
        "UPDATE hoadon SET TrangThai = 'Đã hủy'
         WHERE MaHoaDon = ? AND TenDangNhap = ? AND TrangThai = 'Chưa xác nhận'"
    );
    $stmt->bind_param('is', $ma_hd, $username);

    if ($stmt->execute() && $stmt->affected_rows > 0) {
        echo json_encode(['success' => true, 'message' => 'Đã hủy đơn hàng #' . $ma_hd . ' thành công!']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Hủy đơn thất bại. Vui lòng thử lại!']);
    }
    exit;
}
...
$badge = [
    'Hoàn thành'     => ['class' => 'badge-success', 'icon' => 'fa-circle-check',  'label' => 'Hoàn thành'],
    'Đã xác nhận'    => ['class' => 'badge-info',    'icon' => 'fa-circle-check',  'label' => 'Đã xác nhận'],
...
<div class="profile-wrapper">
    ...
    <main class="profile-main">
        ...
        <div class="topbar" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
            ...
            <a href="../cart.php" class="btn btn-primary" style="white-space:nowrap;text-decoration:none;display:inline-flex;align-items:center;gap:6px;padding:10px 18px;border-radius:8px;background:var(--primary);color:#fff;font-weight:600;font-size:.9rem;">
                <i class="fa-solid fa-cart-shopping"></i> Đi đến giỏ hàng
            </a>
        </div>
        ...
        <section class="content-section" id="section-orders" style="display:none;">
            ...
            <div class="stat-cards">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <div class="stat-value"><?= count(array_filter($orders, fn($o)=>$o['TrangThai']==='Hoàn thành')) ?></div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <div class="stat-value"><?= count(array_filter($orders, fn($o)=>in_array($o['TrangThai'],['Đang giao','Đã xác nhận','Chưa xác nhận']))) ?></div>
...
<div class="modal-overlay" id="orderDetailModal" role="dialog" aria-modal="true" aria-labelledby="orderDetailTitle">
    <div class="modal-box modal-box-lg">
        ...
        <!-- Action buttons (Hủy đơn, etc.) -->
        <div id="orderDetailActions" style="margin-top:16px;text-align:right;display:none;"></div>
    </div>
</div>
