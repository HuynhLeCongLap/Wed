...
try {
    ...
    // 7e. Ghi lịch sử trạng thái đơn hàng
    $sqlTracking = "INSERT INTO order_tracking (MaHoaDon, TrangThai, GhiChu) VALUES (?, 'Chưa xác nhận', 'Đơn hàng vừa được đặt')";
    $stmtT = mysqli_prepare($conn, $sqlTracking);
    if ($stmtT) {
        mysqli_stmt_bind_param($stmtT, 'i', $maHoaDon);
        mysqli_stmt_execute($stmtT);
        mysqli_stmt_close($stmtT);
    }

    // 7f. (Tùy chọn) Cập nhật tổng tiền thanh toán của khách hàng
